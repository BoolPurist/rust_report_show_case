...
impl<T> Node<T> {
    ...
    pub fn get_left_child(&self) -> Option<RootNode<T>> {
        self.left.as_ref().map(|node| Rc::clone(node))
    }

    pub fn get_right_child(&self) -> Option<RootNode<T>> {
...
mod testing {
    ...
    fn should_left_add() {
        ...
        let actual_left_child = root.borrow().get_left_child();
        let actual_right_child = root.borrow().get_right_child();
    ...
    fn should_add_and_remove_left() {
        ...
        match borrowed_root.get_left_child() {
